...
class ActiveMenuRecyclerSingleselectAdapter (
    private val menuList : ArrayList<MenuDC>,
    private val id : String,
    private val itemSelectedList : ArrayList<String>,
    private val showMenuCart: (Boolean) -> Unit
):RecyclerView.Adapter<ActiveMenuRecyclerSingleselectAdapter.SingleselectViewHolder>() {
    ...
    */
     ...
     */
    fun addSelectedItem(){
        database = FirebaseDatabase.getInstance().getReference("Restaurants/" + id)
        var key : String? = null
        for (i in itemSelectedList) {
            Log.d(TAG, "strict run once")
            key = i
        }
        database.child("activeMenu").setValue(key).addOnSuccessListener {
